...
model {
  ...
  for (i in 1:S) {
    ...
    if (i == 1) {
      N[i] ~ birthDeathLike(n0, T[i], lambda[i], mu);
    } else {
      N[i] ~ birthDeathLike(N[i-1], T[i] - T[i-1], lambda[i], mu);
    }
  }
}
...
generated quantities {
  ...
  for (i in 1:S) {
    if (i == 1) {
      N_rep[i] = birthDeathLike_rng(n0, T[i], lambda[i], mu);
    } else {
      N_rep[i] = birthDeathLike_rng(N[i-1], T[i] - T[i-1], lambda[i], mu);
    }
  }
}
