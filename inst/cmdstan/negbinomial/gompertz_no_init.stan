functions {
  ...
  real mean_t(real t, real t0, real n0, real K, vector t_array, vector rho_array) {
    real safe_ratio = fmax(fmin(n0 / K, 1 - 1e-8), 1e-8);
    real C = log(-log(safe_ratio));
    real exponent = fmax(fmin(C - integrated_r(t, t0, t_array, rho_array), 50), -50);
    return K * exp(-exp(exponent));
  }
}
