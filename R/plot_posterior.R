plot_posterior <- function(x, x_fit, par_name, color = "black") {
  ...
  d <- biPOD:::get_parameter(x_fit, par_name)
  p <- ggplot2::ggplot() +
...
plot_posteriors <- function(x, x_fit, par_list, with_histogram = F, alpha = .6) {
  samples <- biPOD:::get_parameters(x_fit, par_list = par_list)
  ...
  p <- ggplot2::ggplot() +
    ...
    ggplot2::scale_fill_manual(values = biPOD:::get_group_colors()[1:length(par_list)]) +
    ggplot2::scale_color_manual(values = biPOD:::get_group_colors()[1:length(par_list)]) +
    biPOD:::my_ggplot_theme()
