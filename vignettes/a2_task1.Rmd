...
If you want to fit one specific type of growth (`exponential` or `logistic`) you have to pass one of the two to the parameter `growth_type`. Otherwise, if you want to test both of them and choose the best one you have to set the `model_selection` parameter to `TRUE`.

Sometimes it might be helpful to scale the input data by a `factor_size`, so that all the counts will be divided by it.

Another important parameter is `t0_lower_bound` which sets the lower bound for the instant of time in which the population was born. If the inserted value is smaller than the minimal x-coordinate in the input dataset a so called $t_0$ will be inferred; if not, this additional parameter will not be inferred. This might be useful when the observed population is decreasing or growing extremely slowly (growth rate near 0).
...
x <- biPOD::fit(
  ...
  factor_size = 1,
  t0_lower_bound = min(x$counts$time) - 50,
  variational = T
)
```
...
Let's use `plot_posteriors` function to plot all the growth rates one agains each other.
