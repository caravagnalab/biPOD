...
```{r}
mouse_id <- 537
d <- xenografts %>% dplyr::filter(mouse == mouse_id) %>% dplyr::mutate(id = mouse)
x <- biPOD::init(
  counts = d,
  break_points = NULL
)
...
Now, to fit the data for the first task one has to use the `fit_growth` function. 

If you want to fit specific type of growths (`exponential` or `logistic`) you have to pass those to the `models_to_fit` paramater.

When a population is decreasing or growing extremely slowly ($\rho \sim 0$) the instant of time in which the population was born might be impossible to infer. To deal with such situations, or when $t_0$ is of no interest, just set the `with_initiation` parameter to `FALSE`. 
...
```{r, warning=F}
biPOD::plot_trace_parameters(x$growth_fit$fit)
```
