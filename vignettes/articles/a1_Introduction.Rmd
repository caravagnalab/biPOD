...
```{r evo plot}
biPOD::plot_input(bipod, add_highlights = T, log_scale = F) 
```

If different groups of parameters are needed and one knows the chaning points in the evolution, one need to pass them in the `init` function.

```{r}
counts <- biPOD::sim_stochastic_exponential(n0 = 10, lambda = .5, mu = c(.4, .45, .3), steps = 3, delta_t = .1)
break_poins <- c(.1, .2, .3)
sample_name <- "Exponential sample"
bipod <- biPOD::init(counts, sample = sample_name, break_points = break_poins)
biPOD::plot_input(bipod, add_highlights = T)
```
