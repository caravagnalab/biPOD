...
d <- biPOD::sim_stochastic_exponential(1000, .8, .2, steps = time_steps, delta_t = delta_t)
```
...
times.plot <- biPOD::plot_threshold_crossing_time_distribution(x25, n0 * 10) + 
...
```{r, warning=FALSE}
fit.plot <- biPOD::plot_fit(x50, final_time = T * 1.01)
# Add the unused points
unused_data <- d[n_points_to_use + 1:nrow(d),] %>% na.omit()
fit.plot <- fit.plot + ggplot2::geom_point(unused_data, mapping=ggplot2::aes(x=time, y=count), alpha = .2)
times.plot <- biPOD::plot_threshold_crossing_time_distribution(x50, n0 * 10) +
  ggplot2::scale_x_continuous(limits = c(2,6))
...
times.plot <- biPOD::plot_threshold_crossing_time_distribution(x100,  n0 * 10) +
  ggplot2::scale_x_continuous(limits = c(2,6))
